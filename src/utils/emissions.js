...
// kg CO₂e / year — regional benchmarks
...
export const PARIS_TARGET_2050 = 500; // kg CO₂e / year per person
...
export function calcTransport({
  ...
  const weekly =
    (FACTORS.transport[carType] ?? FACTORS.transport.car_petrol) * carKmPerWeek +
...
export function calcTotal({ transport = 0, energy = 0, food = 0, shopping = 0 } = {}) {
  return transport + energy + food + shopping;
}

export function getBreakdown(transport, energy, food, shopping) {
...
export function parisGap(totalKgPerYear) {
  const gap = totalKgPerYear - PARIS_TARGET_2050;
  return {
    gap: Math.max(0, gap),
    onTrack: gap <= 0,
    reductionNeeded: Math.max(0, Math.round((gap / totalKgPerYear) * 100)),
  };
}
