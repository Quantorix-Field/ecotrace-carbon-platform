/**
 * Action library for personalised carbon reduction recommendations.
 * Each action has a trigger predicate and saving estimator — both pure functions.
 * Sourced from IPCC AR6 WG3, IEA 2024, and peer-reviewed lifecycle assessments.
 */
...
interface ActionDefinition {
  id: string;
  category: Category;
  trigger: (inputs: Partial<TransportInputs & EnergyInputs & FoodInputs & ShoppingInputs>) => boolean;
  title: string;
  description: string;
  saving: (inputs: Partial<TransportInputs & EnergyInputs & FoodInputs & ShoppingInputs>) => number;
  impact: ImpactLevel;
  effort: ImpactLevel;
...
const ACTION_LIBRARY: ActionDefinition[] = [
  // Transport
  {
    id: "t1",
    category: "transport",
    trigger: (t) => (t.carKmPerWeek ?? 0) > 50 && t.carType === "car_petrol",
    ...
    saving: (t) => Math.round((0.192 - 0.053) * (t.carKmPerWeek ?? 0) * 52),
    impact: "high",
    effort: "high",
    timeframe: "long-term",
  },
  {
    ...
    saving: (t) => Math.round(0.192 * (t.carKmPerWeek ?? 0) * 0.4 * 52),
    impact: "high",
    effort: "low",
    timeframe: "immediate",
  },
  {
    ...
    saving: (t) =>
      Math.round(0.192 * Math.min((t.carKmPerWeek ?? 0) * 0.3, 30) * 52),
    impact: "medium",
    effort: "low",
    timeframe: "immediate",
  },
  {
    id: "t4",
    category: "transport",
    trigger: (t) => (t.flightShortPerYear ?? 0) > 2,
    ...
    saving: (t) =>
      Math.round(((t.flightShortPerYear ?? 0) - 2) * 1200 * 0.255),
    impact: "high",
    effort: "medium",
  ...
  },
  {
    id: "t5",
    category: "transport",
    trigger: (t) => (t.motorbikeKmPerWeek ?? 0) > 50,
    ...
    saving: (t) =>
      Math.round((0.103 - 0.041) * (t.motorbikeKmPerWeek ?? 0) * 0.5 * 52),
    impact: "medium",
    effort: "low",
    timeframe: "immediate",
  },
  ...
  // Energy
  {
    id: "e1",
    category: "energy",
    trigger: (e) => (e.electricityKwhPerMonth ?? 0) > 200,
    ...
    saving: (e) =>
      Math.round(
        Math.min((e.electricityKwhPerMonth ?? 0) * 12 * 0.5, 2400) * 0.716
      ),
    impact: "high",
    effort: "high",
    timeframe: "long-term",
  },
  {
    id: "e2",
    category: "energy",
    trigger: (e) => (e.electricityKwhPerMonth ?? 0) > 100,
  ...
  },
  {
    id: "e3",
    category: "energy",
    trigger: (e) => (e.electricityKwhPerMonth ?? 0) > 150,
    ...
    saving: (e) =>
      Math.round((e.electricityKwhPerMonth ?? 0) * 0.15 * 12 * 0.716),
    impact: "medium",
    effort: "low",
    timeframe: "immediate",
  },
  {
    ...
    saving: (e) =>
      Math.round((e.lpgKgPerMonth ?? 0) * 12 * 1.51 * 0.5),
    impact: "medium",
    effort: "medium",
    timeframe: "short-term",
  },
  {
    id: "e5",
    category: "energy",
    trigger: (e) => (e.electricityKwhPerMonth ?? 0) > 80,
    ...
    saving: (e) =>
      Math.round((e.electricityKwhPerMonth ?? 0) * 0.07 * 12 * 0.716),
    impact: "low",
    effort: "low",
    timeframe: "immediate",
  },
  ...
  // Food
  {
    id: "f1",
    category: "food",
    trigger: (f) => (f.beefKgPerWeek ?? 0) > 0.3,
    ...
    saving: (f) =>
      Math.round((27.0 - 6.9) * (f.beefKgPerWeek ?? 0) * 0.4 * 52),
    impact: "high",
    effort: "low",
    timeframe: "immediate",
  },
  {
    id: "f2",
    category: "food",
    trigger: (f) => (f.beefKgPerWeek ?? 0) + (f.lambKgPerWeek ?? 0) > 0.5,
    ...
    saving: (f) =>
      Math.round(
        ((f.beefKgPerWeek ?? 0) * 27 + (f.lambKgPerWeek ?? 0) * 39.2) *
          0.14 *
          52
      ),
    impact: "high",
  ...
  },
  {
    id: "f3",
    category: "food",
    trigger: (f) => (f.dairyKgPerWeek ?? 0) > 2,
    ...
    saving: (f) =>
      Math.round((f.dairyKgPerWeek ?? 0) * 0.5 * (3.2 - 0.9) * 52),
    impact: "medium",
    effort: "low",
    timeframe: "immediate",
  },
  {
    id: "f4",
    category: "food",
    trigger: (f) => (f.vegetablesKgPerWeek ?? 0) < 2,
  ...
  // Shopping
  {
    id: "s1",
    category: "shopping",
    trigger: (s) => (s.clothingItemsPerYear ?? 0) > 10,
    ...
    saving: (s) =>
      Math.round(((s.clothingItemsPerYear ?? 0) - 5) * 20 * 0.8),
    impact: "high",
    effort: "low",
    timeframe: "immediate",
  },
  {
    id: "s2",
    category: "shopping",
    trigger: (s) => (s.electronicsPerYear ?? 0) > 1,
    ...
    saving: (s) =>
      Math.round(((s.electronicsPerYear ?? 0) - 1) * 70 * 0.5),
    impact: "high",
    effort: "low",
    timeframe: "immediate",
  },
  {
    id: "s3",
    category: "shopping",
    trigger: (s) => (s.plasticKgPerMonth ?? 0) > 1,
    ...
    saving: (s) =>
      Math.round((s.plasticKgPerMonth ?? 0) * 12 * 6.0 * 0.6),
    impact: "medium",
    effort: "low",
    timeframe: "immediate",
  },
  {
    id: "s4",
    category: "shopping",
    trigger: (s) => (s.clothingItemsPerYear ?? 0) > 5,
    ...
    saving: (s) =>
      Math.round((s.clothingItemsPerYear ?? 0) * 20 * 0.3),
    impact: "medium",
    effort: "medium",
    timeframe: "short-term",
  },
...
/**
 * Generates a personalised list of carbon reduction actions based on user inputs.
 * Actions are filtered by trigger predicates and sorted by impact tier,
 * then by estimated saving within each tier.
 * @param transportInputs - User's transport activity data
 * @param energyInputs - User's energy consumption data
 * @param foodInputs - User's dietary data
 * @param shoppingInputs - User's shopping and consumption data
 * @returns Sorted array of matched actions with estimated savings
 */
export function generateInsights(
  transportInputs: Partial<TransportInputs> | null,
  energyInputs: Partial<EnergyInputs> | null,
  foodInputs: Partial<FoodInputs> | null,
  shoppingInputs: Partial<ShoppingInputs> | null
): Action[] {
  const t = transportInputs ?? {};
  const e = energyInputs ?? {};
  const f = foodInputs ?? {};
  const s = shoppingInputs ?? {};

  const matched = ACTION_LIBRARY.filter((action) => {
    try {
      switch (action.category) {
        case "transport": return action.trigger(t);
        case "energy":    return action.trigger(e);
        case "food":      return action.trigger(f);
        case "shopping":  return action.trigger(s);
  ...
  return matched
    .map((action) => {
      ...
      try {
        switch (action.category) {
          case "transport": saving = action.saving(t); break;
          case "energy":    saving = action.saving(e); break;
          case "food":      saving = action.saving(f); break;
          case "shopping":  saving = action.saving(s); break;
        }
      } catch {
        saving = 0;
      }
...
/**
 * Returns the top N actions sorted by impact and estimated saving.
 * @param transportInputs - User's transport activity data
 * @param energyInputs - User's energy consumption data
 * @param foodInputs - User's dietary data
 * @param shoppingInputs - User's shopping and consumption data
 * @param n - Maximum number of actions to return (default: 5)
 * @returns Top N matched actions
 */
...
/**
 * Calculates the total potential CO₂e saving across all matched actions.
 * @param insights - Array of matched actions with estimated savings
 * @returns Total potential saving in kg CO₂e per year
 */
export function totalPotentialSaving(insights: Partial<Action>[]): number {
