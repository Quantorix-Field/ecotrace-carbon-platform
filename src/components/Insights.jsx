...
export default function Insights({
  ...
  if (allInsights.length === 0) {
    return (
      <div className="section" style={{ background: "var(--green-50)", borderTop: "1px solid var(--color-border)" }}>
  ...
  return (
    <div className="section" style={{ background: "var(--green-50)", borderTop: "1px solid var(--color-border)" }}>
      <div className="container">
        ...
        <div
          style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "var(--space-4)", marginBottom: "var(--space-8)" }}
        ...
        {filtered.length === 0 ? (
          <div className="card" style={{ textAlign: "center", padding: "var(--space-10)", color: "var(--gray-500)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
            {filtered.map((action, i) => (
              <ActionCard
                ...
                onToggle={() => setExpanded((prev) => (prev === action.id ? null : action.id))}
              />
            ))}
          </div>
        )}
...
function SummaryTile({ emoji, value, label, bg, light }) {
  return (
    <div className="card" style={{ background: bg, border: "none", textAlign: "center", padding: "var(--space-5)" }}>
      <div style={{ fontSize: "1.75rem", marginBottom: "var(--space-2)" }}>{emoji}</div>
      <div style={{ fontSize: "1.75rem", fontWeight: 800, color: light ? "#fff" : "var(--gray-900)", lineHeight: 1, marginBottom: "var(--space-1)" }}>
        {value}
      </div>
...
function ActionCard({ action, index, isExpanded, onToggle }) {
  ...
  return (
    <div
      className="card"
      style={{
        borderLeft: `4px solid ${action.impact === "high" ? "var(--green-500)" : action.impact === "medium" ? "#f59e0b" : "var(--gray-300)"}`,
        cursor: "pointer",
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "var(--space-4)" }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "var(--space-2)", marginBottom: "var(--space-2)", flexWrap: "wrap" }}>
            <span className={`badge badge-${action.impact}`}>{action.impact} impact</span>
            <span style={{ fontSize: "0.75rem", color: effort.color, fontWeight: 600 }}>{effort.text}</span>
            <span style={{ fontSize: "0.75rem", color: "var(--gray-400)", textTransform: "capitalize" }}>
              · {action.timeframe.replace("-", " ")}
            </span>
          </div>
          <h4 style={{ fontSize: "0.9375rem", fontWeight: 600, color: "var(--gray-900)", lineHeight: 1.4 }}>
            {action.title}
          </h4>
        </div>

        <div style={{ textAlign: "right", flexShrink: 0 }}>
          <div style={{ background: "var(--green-50)", border: "1px solid var(--green-200)", borderRadius: "var(--radius-full)", padding: "var(--space-1) var(--space-3)", fontSize: "0.8125rem", fontWeight: 700, color: "var(--green-700)", whiteSpace: "nowrap" }}>
            −{(action.estimatedSaving / 1000).toFixed(2)}t/yr
          </div>
          <div style={{ fontSize: "0.7rem", color: "var(--gray-400)", marginTop: "var(--space-1)", textAlign: "right" }}>
            CO₂e saved
          </div>
        </div>
      </div>
      ...
      {isExpanded && (
        <div
          style={{ marginTop: "var(--space-4)", paddingTop: "var(--space-4)", borderTop: "1px solid var(--color-border)", animation: "fadeIn var(--transition-fast) ease" }}
          onClick={(e) => e.stopPropagation()}
        >
          <p style={{ fontSize: "0.9375rem", color: "var(--gray-600)", lineHeight: 1.7, margin: 0 }}>
            {action.description}
          </p>
...
function MetaPill({ label, value }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
      <span style={{ fontSize: "0.7rem", color: "var(--gray-400)", textTransform: "uppercase", letterSpacing: "0.05em" }}>{label}</span>
      <span style={{ fontSize: "0.8125rem", fontWeight: 600, color: "var(--gray-700)", textTransform: "capitalize" }}>{value}</span>
    </div>
  );
}

const inputShape = PropTypes.object;

Insights.propTypes = {
  transportInputs: inputShape.isRequired,
  energyInputs:    inputShape.isRequired,
  foodInputs:      inputShape.isRequired,
  shoppingInputs:  inputShape.isRequired,
